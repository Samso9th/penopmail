...
import { formatAmount, PRICES, type Region } from "@/lib/region";
...
export function OnboardingClient({ region }: { region: Region }) {
  ...
  const minAmount = domains.some((d) => d.type === "custom")
    ? p.customRaw
    : p.sharedRaw;
  ...
  const validateAmount = (val: string) => {
    ...
    if (!val || isNaN(num) || num < minAmount) {
      setAmountError(`Minimum amount is ${formatAmount(region, minAmount)}`);
      return false;
    }
    setAmountError("");
    return true;
        ...
        <Card className="w-full max-w-md">
          ...
          {step === 1 && (
            <>
              ...
              <CardContent className="space-y-5">
                <div className="bg-muted space-y-1.5 rounded-lg p-4 text-sm">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      className="pl-9"
                      type="number"
                      step={region === "intl" ? "0.01" : "1"}
                      placeholder={`Min ${formatAmount(region, minAmount)}`}
                      value={amount}
                      onChange={(e) => {
                ...
                <div className="bg-muted space-y-1.5 rounded-lg p-4 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-primary font-semibold">
                      {formatAmount(region, parseFloat(amount) || 0)}
                    </span>
                  </div>
                </div>
                <Button
